...
class SceaTaskScreen extends ConsumerStatefulWidget {
  final String firstName;
  final String lastName;
  final bool isCorrection;
  // Paramètre supprimé: showAlreadyEnteredMessage

  const SceaTaskScreen({
    super.key,
  ...
  @override
  ConsumerState<SceaTaskScreen> createState() => _SceaTaskScreenState();
}

class _SceaTaskScreenState extends ConsumerState<SceaTaskScreen> with TickerProviderStateMixin {
  ...
  final Map<String, List<Map<String, dynamic>>> _sceaTaskCategories = {
    'Vignoble': [
      {'name': 'Taille', 'selected': false, 'hours': ''},
      {'name': 'Palissage débourgeonnage', 'selected': false, 'hours': ''},
      {'name': 'Vendanges', 'selected': false, 'hours': '', 'night_hours': ''},
      {'name': 'Traitement Bertoni', 'selected': false, 'hours': '', 'night_hours': ''},
      {'name': 'Traitement pulvé desherbage', 'selected': false, 'hours': '', 'night_hours': ''},
      {'name': 'Travail sol labour compost engrais', 'selected': false, 'hours': ''},
      {'name': 'Plantier', 'selected': false, 'hours': ''},
      {'name': 'Entretien palissage', 'selected': false, 'hours': ''},
      {'name': 'Irrigation', 'selected': false, 'hours': ''},
    ],
    'Cave & Production': [
      {'name': 'Travaux de cave', 'selected': false, 'hours': ''},
      {'name': 'Decuvage', 'selected': false, 'hours': ''},
      {'name': 'Mise en bouteille', 'selected': false, 'hours': ''},
      {'name': 'Étiquetage', 'selected': false, 'hours': ''},
      {'name': 'Barriques', 'selected': false, 'hours': ''},
    ],
    'Matériel & Transport': [
      {'name': 'Tracteur', 'selected': false, 'hours': ''},
      {'name': 'Maintenance matériel', 'selected': false, 'hours': ''},
      {'name': 'Travaux mécaniques atelier', 'selected': false, 'hours': ''},
    ],
    'Administratif': [
  ...
  void initState() {
    ...
    if (_sceaTaskCategories.containsKey('Kilométrage')) {
      _sceaTaskCategories['Kilométrage'] = [];
    }

    super.initState();
    
    // Préparer les données pour l'interface
    _categoryNames = _sceaTaskCategories.keys.toList();
    _categoryTasks = _sceaTaskCategories.values.toList();
  ...
  Future<void> _initData() async {
    ...
    try {
      ...
      taskNotifier.initCompanyTasks('SCEA', flatTasks);
  ...
  Future<void> _saveData() async {
    ...
    for (int i = 0; i < _categoryNames.length; i++) {
      ...
      for (var task in categoryTasks) {
        final savedTask = taskNotifier.getCompanyTasks('SCEA')
    ...
    if (confirm == true) {
      try {
        ...
        await prefs.setString('selectedCompany', 'SCEA');
          ...
          children: [
            Row(
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    taskNotifier.toggleCompanyTask('SCEA', task['name']);
                  },
                  child: Container(
                    width: 24,
                    height: 24,
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: TextFormField(
                      ...
                      onChanged: (value) {
                        taskNotifier.updateCompanyTaskHours('SCEA', task['name'], value);
                      },
                    ),
                  ),
                ],
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (value) {
                          ...
                          final tasks = taskNotifier.getCompanyTasks('SCEA');
                          for (var t in tasks) {
                            if (t['name'] == task['name']) {
                              t['night_hours'] = value;
                              break;
  ...
  Widget _buildCategoryContent(int categoryIndex) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final savedTask = ref.read(taskProvider.notifier).getCompanyTasks('SCEA')
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Saisie des heures - SCEA'),
